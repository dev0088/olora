...
class LinkController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $addLink = Link::create([
                ...
                'image'             => $urlData['image'],
                'origin_query'      => $urlData['searchQuery']
            ]);
    ...
    public function update(LinkRequest $request, $id)
    {
        ...
        try {
            ...
            $link->utm_campaign_id  = $request->utm_campaign_id;
            $link->utm_source_id    = $request->utm_source_id;
            $link->utm_medium_id    = $request->utm_medium_id;
            $link->utm_content_id   = $request->utm_content_id;
            $link->utm_term_id      = $request->utm_term_id;

            $old_values = $link->getOriginal();
            $link->save();

            // Handle UTM Parameter url counts
            $link->handleUtmAnalysis($old_values);
    ...
    /**
     * Reset the total click counts of link to 0
     */
    public function resetClicks(Request $request, $id)
    {
        try {
            ...
            return response(['message' => 'Click counts of your link status was successfully reseted'], 201);

        }
        catch (Exception $e) {
            return response(['message' => 'Unable to reset click counts of your link at the moment'], 500);
        }
    }
